...
func StartDeepResearchHandler(logger *logger.Logger, trackingService *request_tracking.Service, firebaseClient *auth.FirebaseClient, storage MessageStorage, sessionManager *SessionManager, queries pgdb.Querier, deepResearchRateLimitEnabled bool, notificationService *notifications.Service, titleService *title_generation.Service, modelRouter *routing.ModelRouter) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Check if this is the first deep research run for this chat (for title generation)
		// Note: This query will fail if sqlc hasn't been run yet after adding the new SQL query
		// TODO: Run `make sqlc` after merging this change
		if titleService != nil && modelRouter != nil {
			// Check if any runs exist for this chat (will be 0 on first run)
			// Note: GetDeepResearchRunCountForChat is generated by sqlc - if build fails, run `make sqlc`
			runCount, err := queries.GetDeepResearchRunCountForChat(c.Request.Context(), pgdb.GetDeepResearchRunCountForChatParams{
				UserID: userID,
				ChatID: req.ChatID,
			})
			if err != nil {
				log.Warn("failed to get run count for chat (title generation check)",
					slog.String("error", err.Error()),
					slog.String("user_id", userID),
					slog.String("chat_id", req.ChatID))
			} else if runCount == 0 {
