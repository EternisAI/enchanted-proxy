...
func (c *LinearClient) AddComment(ctx context.Context, issueID, comment string) error {
	query := `
	...
	body, err := json.Marshal(reqBody)
	if err != nil {
		return fmt.Errorf("failed to marshal Linear comment request: %w", err)
	}

	req, err := http.NewRequestWithContext(ctx, "POST", "https://api.linear.app/graphql", bytes.NewReader(body))
	if err != nil {
		return fmt.Errorf("failed to create Linear comment request: %w", err)
	}
	...
	if err := json.NewDecoder(resp.Body).Decode(&commentResp); err != nil {
		return fmt.Errorf("failed to decode Linear comment response: %w", err)
	}

	if len(commentResp.Errors) > 0 {
		return fmt.Errorf("Linear API error: %s", commentResp.Errors[0].Message)
	}
